...
async function run() {
  ...
  // ---------------------------------------------------------------------
  {
    const { res } = await withState(async () => {
      const req = makeReq({ token: null, body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'c-1' } });
      const res = makeRes();
  ...
  {
    const { res } = await withState(async () => {
      const req = makeReq({ token: 'valid-token-no-org', body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'c-1' } });
      const res = makeRes();
      await handler(req, res);
      return { res };
    });
  ...
  {
    const { res } = await withState(async () => {
      const req = makeReq({ body: { eventType: 'made_up_event', eventFingerprint: 'fp-shared-account', clientEventId: 'c-1' } });
      const res = makeRes();
      await handler(req, res);
      return { res };
    });
  ...
  {
    const { res } = await withState(async () => {
      const req = makeReq({ body: { eventType: 'outcome_reported', eventFingerprint: 'fp-shared-account', clientEventId: 'c-1' } });
      const res = makeRes();
      await handler(req, res);
      return { res };
    });
  ...
  {
    const { res } = await withState(async () => {
      const req = makeReq({ body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account' } });
      const res = makeRes();
      await handler(req, res);
      return { res };
    });
  ...
  // ---------------------------------------------------------------------
  // Organization-level signal resolution: rejects a fingerprint outside
  // the caller's organization, accepts one owned by a TEAMMATE (not the
  // caller's own row) within the same organization.
  // ---------------------------------------------------------------------
  {
    const { res, state } = await withState(async () => {
      const req = makeReq({ body: { eventType: 'signal_useful', eventFingerprint: 'fp-org-b-only', clientEventId: 'c-cross-org' } });
      const res = makeRes();
      await handler(req, res);
      return { res, state: null };
    });
    assert(res.statusCode === 404, `REQUIRED: a fingerprint belonging to a DIFFERENT organization is rejected 404, not written (got ${res.statusCode})`);
  }
  {
    const { res, ev } = await withState(async (state) => {
      const req = makeReq({ token: 'valid-token-a1', body: { eventType: 'signal_selected', eventFingerprint: 'fp-teammate-only', accountName: 'attacker-supplied-account-name', clientEventId: 'c-teammate' } });
      const res = makeRes();
      await handler(req, res);
      return { res, ev: state.haSignalEvents[0] };
    });
    assert(res.statusCode === 200, `REQUIRED: a fingerprint owned by a TEAMMATE in the same organization resolves successfully (org-level, not current-user-only) (got ${res.statusCode})`);
  ...
  // ---------------------------------------------------------------------
  {
    const { ev } = await withState(async (state) => {
      const req = makeReq({ body: {
        eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'c-snapshot',
  ...
  {
    const { ev } = await withState(async (state) => {
      const req = makeReq({ body: { eventType: 'prepare_call_opened', eventFingerprint: 'fp-shared-account', clientEventId: 'c-open' } });
      const res = makeRes();
      await handler(req, res);
      return { ev: state.haSignalEvents[0] };
    });
  ...
  // ---------------------------------------------------------------------
  {
    const { firstId, secondId, count } = await withState(async (state) => {
      const req1 = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-retry-same' } });
      const res1 = makeRes();
      await handler(req1, res1);
      const req2 = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-retry-same' } });
      const res2 = makeRes();
  ...
  // ---------------------------------------------------------------------
  {
    const { firstRes, repeatRes, changeRes, count, types } = await withState(async (state) => {
      const req1 = makeReq({ body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'c-useful-1' } });
      const res1 = makeRes();
      await handler(req1, res1);

      // A genuinely new gesture (different clientEventId), same judgment.
      const req2 = makeReq({ body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'c-useful-2-different-id' } });
      const res2 = makeRes();
      await handler(req2, res2);

      // Now a changed judgment.
      const req3 = makeReq({ body: { eventType: 'signal_not_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'c-not-useful-1' } });
      const res3 = makeRes();
  ...
  // ---------------------------------------------------------------------
  {
    const { count, ids } = await withState(async (state) => {
      const req1 = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-outreach-1' } });
      const res1 = makeRes();
      await handler(req1, res1);
      const req2 = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-outreach-2' } });
      const res2 = makeRes();
      await handler(req2, res2);
  ...
  // ---------------------------------------------------------------------
  {
    const { noteRes, ev } = await withState(async (state) => {
      const outreachReq = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-outreach-for-note' } });
  ...
  }
  {
    ...
    const { res } = await withState(async (state) => {
      const outreachReq = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-outreach-empty-note' } });
      const outreachRes = makeRes();
      await handler(outreachReq, outreachRes);
  ...
  }
  {
    // Length limit.
    const { res } = await withState(async (state) => {
      const outreachReq = makeReq({ body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'c-outreach-long-note' } });
      const outreachRes = makeRes();
      await handler(outreachReq, outreachRes);
  ...
  }
  {
    ...
    const { res } = await withState(async (state) => {
      const outreachReq = makeReq({ token: 'valid-token-a2', body: { eventType: 'outreach_made', eventFingerprint: 'fp-teammate-only', clientEventId: 'c-a2-outreach' } });
      const outreachRes = makeRes();
      await handler(outreachReq, outreachRes);
  ...
  // ---------------------------------------------------------------------
  {
    const { states } = await withState(async (state) => {
      ...
      await handler(makeReq({ token: 'valid-token-a1', body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'r-useful' } }), makeRes());
      const notUsefulRes = makeRes();
      await handler(makeReq({ token: 'valid-token-a1', body: { eventType: 'signal_not_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'r-not-useful' } }), notUsefulRes);
      const outreachRes = makeRes();
      await handler(makeReq({ token: 'valid-token-a1', body: { eventType: 'outreach_made', eventFingerprint: 'fp-shared-account', clientEventId: 'r-outreach' } }), outreachRes);
      ...
      // Rep A2 (teammate, same org): marks the SAME fingerprint useful --
      // must never leak into rep A1's own read-back.
      await handler(makeReq({ token: 'valid-token-a2', body: { eventType: 'signal_selected', eventFingerprint: 'fp-shared-account', clientEventId: 'r-a2-select' } }), makeRes());
      await handler(makeReq({ token: 'valid-token-a2', body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'r-a2-useful' } }), makeRes());

      const getRes = makeRes();
      await handler(makeReq({ method: 'GET', token: 'valid-token-a1', query: { eventFingerprints: 'fp-shared-account,fp-nonexistent' } }), getRes);
      return { states: getRes.body.states };
    });
    const s = states['fp-shared-account'];
    assert(!!s, 'sanity: the requested fingerprint has a state entry');
    ...
    assert(states['fp-nonexistent'] && states['fp-nonexistent'].feedback === null && states['fp-nonexistent'].outreachLogged === false, 'a fingerprint with no history returns a clean, unanswered default state');
  }
  {
    // Cross-rep isolation, explicit: rep A2's own read-back for the same
    // fingerprint must reflect ONLY rep A2's history, not rep A1's.
    const { states } = await withState(async (state) => {
      await handler(makeReq({ token: 'valid-token-a1', body: { eventType: 'signal_not_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'iso-a1' } }), makeRes());
      await handler(makeReq({ token: 'valid-token-a2', body: { eventType: 'signal_useful', eventFingerprint: 'fp-shared-account', clientEventId: 'iso-a2' } }), makeRes());
      const getRes = makeRes();
      await handler(makeReq({ method: 'GET', token: 'valid-token-a2', query: { eventFingerprints: 'fp-shared-account' } }), getRes);
      return { states: getRes.body.states };
    });
    assert(states['fp-shared-account'].feedback === 'useful', `REQUIRED: rep A2's read-back reflects ONLY their own judgment (useful), never rep A1's (not_useful) for the same signal (got ${states['fp-shared-account'].feedback})`);
  }
  {
    const { res } = await withState(async () => {
    ...
    assert(res.statusCode === 400, `GET with no eventFingerprints is rejected 400 (got ${res.statusCode})`);
  }
  {
    const { res } = await withState(async () => {
      const req = makeReq({ method: 'GET', token: null, query: { eventFingerprints: 'fp-shared-account' } });
      const res = makeRes();
      await handler(req, res);
      return { res };
    });
