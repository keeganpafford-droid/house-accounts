...
const REAL_SOURCE = [
  extractRaw('ACTIVE_RESEARCH_BREADCRUMB_KEY', 5632, 5632, 'const ACTIVE_RESEARCH_BREADCRUMB_KEY'),
  extractFn('setActiveResearchBreadcrumb', 5633, 5639),
  extractFn('clearActiveResearchBreadcrumb', 5640, 5651),
  extractFn('getActiveResearchBreadcrumb', 5652, 5659),
  extractFn('applyModalResearchResultToDashboard', 6611, 6621),
  extractFn('normalizeAccountNameForKey', 6626, 6626),
  extractFn('recentlyResearchedKey', 6631, 6631),
  extractFn('findTimeboxForAccountOpportunity', 6639, 6646),
  extractFn('highlightResultElement', 6648, 6654),
  extractFn('scrollToAccountResult', 6668, 6688),
  extractRaw('RECENTLY_RESEARCHED_WINDOW_MS', 6776, 6776, 'const RECENTLY_RESEARCHED_WINDOW_MS'),
  extractRaw('DISMISSED_RESEARCH_STORAGE_PREFIX', 6805, 6805, 'const DISMISSED_RESEARCH_STORAGE_PREFIX'),
  extractRaw('DISMISSED_RESEARCH_PRUNE_AFTER_MS', 6812, 6812, 'const DISMISSED_RESEARCH_PRUNE_AFTER_MS'),
  extractFn('dismissedResearchNamespace', 6813, 6819),
  extractFn('dismissedResearchStorageKey', 6820, 6820),
  extractFn('readDismissedResearchMap', 6821, 6830),
  extractFn('writeDismissedResearchMap', 6831, 6834),
  extractFn('pruneDismissedResearchMap', 6835, 6842),
  extractFn('isResearchResultDismissed', 6847, 6855),
  extractFn('dismissResearchResult', 6856, 6860),
  extractFn('getRecentlyResearchedAccounts', 6863, 6877),
  extractFn('relativeResearchTimeLabel', 6878, 6885),
  extractFn('renderRecentlyResearchedSection', 6886, 6911),
  extractRaw('recentlyResearchedClickListener', 6912, 6930, "document.addEventListener('click', (event) => {"),
  extractFn('escapeHtml', 9522, 9525),
  extractRaw('modalFmtEsc', 9563, 9565, "const fmt=d=>"),
  extractFn('request', 9574, 9591, {async: true}),
  extractFn('accountRow', 9618, 9659),
  extractFn('researchRunBanner', 9664, 9675),
  extractFn('listCard', 9684, 9716),
  extractRaw('renderManager', 9717, 9717, 'function renderManager(){'),
  extractFn('isModalOpen', 9728, 9731),
  extractFn('anyListHasActiveRun', 9732, 9734),
  extractFn('stopResearchPoll', 9735, 9737),
  extractFn('scheduleResearchPollIfNeeded', 9738, 9742),
  extractFn('load', 9743, 9752, {async: true}),
  extractRaw('openClose', 9754, 9767, "function open(){"),
  extractFn('showInfoDialog', 10179, 10218)
].join('\n\n');
...
{
  const closeSrc = extractRaw('closeOnly', 9768, 9776, "function close(){");
...
{
  const deleteAccountBranch = extractRaw('deleteAccountCatchBranch', 10239, 10268, "if(action==='delete-account'){");
...
// through behavior.
{
  ...
  const handoffSrc = extractRaw('viewOpportunitiesHandoff', 9891, 9910, "if(fresh && typeof applyModalResearchResultToDashboard === 'function') applyModalResearchResultToDashboard(fresh, listId);");
...
async function runClientTests(){
  ...
  // ---------------------------------------------------------------------
  {
    const loadSrc = extractFn('load', 9743, 9752, { async: true });
  ...
  }
  {
    ...
    const src = extractFn('applyModalResearchResultToDashboard', 6611, 6621);
