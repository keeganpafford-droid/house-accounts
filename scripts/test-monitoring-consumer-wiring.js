...
import { QUEUE_VISIBILITY_TIMEOUT_SECONDS, ALREADY_LEASED_RETRY_DELAY_SECONDS } from '../api/lib/monitoring-queue.js';
...
assert(/new QueueClient\(\)/.test(source), 'REQUIRED: the consumer instantiates the real QueueClient class (handleNodeCallback is only available on a QueueClient instance per the installed SDK, not as a bare module export)');
assert(/retry:\s*\(error, metadata\)/.test(source) || /retry:\s*\(error,\s*metadata\)/.test(source), 'REQUIRED: handleNodeCallback is given an explicit retry handler, not a bare throw with no delay control');
assert(/MonitoringRetryError/.test(source), 'REQUIRED (item 3): the consumer signals a delayed retry via MonitoringRetryError, which the retry handler reads afterSeconds off of, rather than acking or throwing an undifferentiated error');
assert(typeof ALREADY_LEASED_RETRY_DELAY_SECONDS === 'number' && ALREADY_LEASED_RETRY_DELAY_SECONDS > 0, `sanity: the already-leased retry delay constant is defined and positive (got ${ALREADY_LEASED_RETRY_DELAY_SECONDS})`);
