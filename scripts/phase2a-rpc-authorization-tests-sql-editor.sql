...
-- (Phase 2A implementation-review, tests 1-58, through ROUND 10). Same tests,
-- same fixtures, same assertions -- mechanically converted so every
-- assertion lands as a ROW in a results table instead of a RAISE NOTICE
-- line, ending in exactly ONE visible SELECT. Generated from the
-- RAISE-NOTICE-based file; if the two ever appear to disagree, the
-- RAISE-NOTICE original is the canonical source (this file is a reporting
-- adaptation of it, not a separate hand-maintained test suite).
--
...
-- through PostgREST).
--
-- Output: every assertion (109 originally-PASS, 142 originally-FAIL-shaped
-- checks, plus 21 cleanup/informational markers -- 272 rows total) is
-- inserted into a temp table `_phase2a_auth_test_results` as it runs. A
-- final block re-derives the starting row counts for ha_users/ha_uploads/
-- ha_accounts/ha_signals/ha_research_runs captured before any test ran,
-- performs one last blanket cleanup sweep (belt-and-suspenders on top of
-- each block's own internal cleanup), and appends one more row confirming
-- counts were restored exactly. The file ends with ONE
-- `select * from _phase2a_auth_test_results order by id;` and then drops the
-- temp table. No RAISE NOTICE is used anywhere in the executable path below.
--
...
-- ===========================================================================
-- Results table + starting-row-count baseline. Created before any test
-- fixture is inserted, so the final restoration check reflects the true
-- pre-run state of the database, not just this file's own fixtures.
-- ===========================================================================
...
begin
  ...
  begin
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 1: ' || 'expected 1 row for the correct owner, got %', false, format('expected 1 row for the correct owner, got %', v_count)); end if;
  exception when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 1: ' || 'correct owner/upload call raised an unexpected exception: %', false, format('correct owner/upload call raised an unexpected exception: %', sqlerrm));
  end;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 2: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 2: ' || '% row(s) were written to upload A despite the rejection', false, format('% row(s) were written to upload A despite the rejection', v_count)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 3: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
...
begin
  ...
  begin
    ...
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 4: ' || 'anon role was able to call replace_ha_accounts_snapshot at all -- EXECUTE grant is not properly revoked', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 4: ' || 'anon role is rejected at the privilege level (insufficient_privilege) before the function body runs', true, null);
  when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 4: ' || 'anon call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', false, format('anon call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', sqlerrm, sqlstate));
  end;
  reset role;
end $$;
...
begin
  ...
  begin
    perform public.replace_ha_accounts_snapshot(gen_random_uuid(), gen_random_uuid(), '[]'::jsonb, 'initial_upload');
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 5: ' || 'authenticated role was able to call replace_ha_accounts_snapshot directly -- EXECUTE grant is not properly revoked', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 5: ' || 'authenticated role is rejected at the privilege level (insufficient_privilege), matching the intended service_role-only design', true, null);
  when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 5: ' || 'authenticated call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', false, format('authenticated call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', sqlerrm, sqlstate));
  end;
  reset role;
end $$;
...
begin
  ...
  begin
    perform public.replace_ha_accounts_snapshot(gen_random_uuid(), gen_random_uuid(), '[]'::jsonb, 'initial_upload');
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 6: ' || 'service_role call unexpectedly succeeded against a nonexistent upload/user (should hit the ownership check, sqlstate 42501)', false, null);
  exception when sqlstate '42501' then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 6: ' || 'service_role CAN call the function (reaches the function body -- rejected here only because the test upload/user do not exist, which is the correct, expected outcome for this specific call)', true, null);
  when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 6: ' || 'service_role itself is blocked at the privilege level -- the GRANT to service_role is missing', false, null);
  end;
  reset role;
end $$;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 7: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 7: ' || '% row(s) exist for this upload despite the rejected malformed call', false, format('% row(s) exist for this upload despite the rejected malformed call', v_count)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 8: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  exception when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 9: ' || 'an in-array duplicate account_name raised an exception instead of being resolved deterministically: % (sqlstate %)', false, format('an in-array duplicate account_name raised an exception instead of being resolved deterministically: % (sqlstate %)', sqlerrm, sqlstate));
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 9: ' || 'expected exactly 1 row for the duplicated account_name, got %', false, format('expected exactly 1 row for the duplicated account_name, got %', v_count)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 11: ' || 'expected 0 accounts remaining after an empty-array call, got %', false, format('expected 0 accounts remaining after an empty-array call, got %', v_count)); end if;
  ...
  if v_distinct_owner_pairs = 1 then insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 12: ' || 'every returned/persisted row for this call carries exactly one (upload_id, user_id) pair -- (%, %)', true, format('every returned/persisted row for this call carries exactly one (upload_id, user_id) pair -- (%, %)', v_upload, v_user));
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 12: ' || 'expected exactly one distinct (upload_id, user_id) pair among the persisted rows, found %', false, format('expected exactly one distinct (upload_id, user_id) pair among the persisted rows, found %', v_distinct_owner_pairs)); end if;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 13: ' || 'expected outcome=claimed-new, got %', false, format('expected outcome=claimed-new, got %', v_result->>'outcome')); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 14: ' || 'expected outcome=attached-active, got %', false, format('expected outcome=attached-active, got %', v_result->>'outcome')); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 14: ' || 'expected exactly 1 row, found %', false, format('expected exactly 1 row, found %', v_count)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 15: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 16: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 17: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  begin
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 18: ' || 'expected outcome=reclaimed-after-expired-lease, got %', false, format('expected outcome=reclaimed-after-expired-lease, got %', v_result->>'outcome')); end if;
    if v_new_attempt <> v_old_attempt then insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 18: ' || 'reclaiming an expired lease mints a NEW attempt_id (% -> %)', true, format('reclaiming an expired lease mints a NEW attempt_id (% -> %)', v_old_attempt, v_new_attempt));
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 19: ' || 'expected outcome=claimed-new for the new run id after the old one''s lease expired, got %', false, format('expected outcome=claimed-new for the new run id after the old one''s lease expired, got %', v_result->>'outcome')); end if;
  ...
  begin
    ...
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 20 (sanity): anon and authenticated cannot call claim_ha_research_run at all: ' || 'anon role was able to call claim_ha_research_run -- EXECUTE grant is not properly revoked', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 20 (sanity): anon and authenticated cannot call claim_ha_research_run at all: ' || 'anon role is rejected at the privilege level before the function body runs', true, null);
  when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 20 (sanity): anon and authenticated cannot call claim_ha_research_run at all: ' || 'anon call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', false, format('anon call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', sqlerrm, sqlstate));
  end;
  reset role;
  set role authenticated;
  begin
    perform public.claim_ha_research_run(gen_random_uuid(), gen_random_uuid(), 'x', 300);
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 20 (sanity): anon and authenticated cannot call claim_ha_research_run at all: ' || 'authenticated role was able to call claim_ha_research_run directly -- EXECUTE grant is not properly revoked', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 20 (sanity): anon and authenticated cannot call claim_ha_research_run at all: ' || 'authenticated role is rejected at the privilege level, matching the intended service_role-only design', true, null);
  when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 20 (sanity): anon and authenticated cannot call claim_ha_research_run at all: ' || 'authenticated call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', false, format('authenticated call raised an unexpected error (expected insufficient_privilege): % (sqlstate %)', sqlerrm, sqlstate));
  end;
  reset role;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 21: ' || 'expected ok=true, got %', false, format('expected ok=true, got %', v_heartbeat->>'ok')); end if;
  if v_lease_2 > v_lease_1 then insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 21: ' || 'heartbeat extends lease_expires_at forward (% -> %)', true, format('heartbeat extends lease_expires_at forward (% -> %)', v_lease_1, v_lease_2));
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 22: ' || 'expected ok=false/not-current-attempt, got %', false, format('expected ok=false/not-current-attempt, got %', v_heartbeat)); end if;
  ...
  begin
    ...
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 24: ' || 'anon role was able to call heartbeat_ha_research_run -- EXECUTE grant is not properly revoked', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 24: ' || 'anon role is rejected at the privilege level before the function body runs', true, null);
  end;
  reset role;
  set role authenticated;
  begin
    perform public.heartbeat_ha_research_run(gen_random_uuid(), gen_random_uuid(), 'x', gen_random_uuid(), 300);
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 24: ' || 'authenticated role was able to call heartbeat_ha_research_run directly -- EXECUTE grant is not properly revoked', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 24: ' || 'authenticated role is rejected at the privilege level, matching the intended service_role-only design', true, null);
  end;
  reset role;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 25: ' || 'expected outcome=completed attached to the manual run, got %', false, format('expected outcome=completed attached to the manual run, got %', v_auto)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 26: ' || 'expected outcome=claimed-new for the explicit manual rerun, got %', false, format('expected outcome=claimed-new for the explicit manual rerun, got %', v_manual)); end if;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 27: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 27: ' || '% row(s) were written despite the rejection', false, format('% row(s) were written despite the rejection', v_count)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 27: ' || 'expected 1 row from the current attempt''s write, got %', false, format('expected 1 row from the current attempt''s write, got %', v_count)); end if;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 28: ' || 'expected accountsPersisted=1, got %', false, format('expected accountsPersisted=1, got %', v_result->>'accountsPersisted')); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 28: ' || 'unexpected signals counts in %', false, format('unexpected signals counts in %', v_result)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 28: ' || 'ha_uploads.stage/summary not updated as expected (got stage=%, summary=%)', false, format('ha_uploads.stage/summary not updated as expected (got stage=%, summary=%)', v_upload_row.stage, v_upload_row.summary)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 29: ' || 'unexpected repeat-signal counts in %', false, format('unexpected repeat-signal counts in %', v_result)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 30: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 31: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  begin
    ...
    exception when others then
      ...
      else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 32: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
    end;
    select count(*) into v_count from public.ha_accounts where upload_id = v_upload_a;
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 32: ' || 'ha_accounts row count changed from % to % despite the rejection', false, format('ha_accounts row count changed from % to % despite the rejection', v_accounts_before, v_count)); end if;
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 32: ' || 'ha_signals row count changed from % to % despite the rejection', false, format('ha_signals row count changed from % to % despite the rejection', v_signals_before, v_count)); end if;
  end;
  ...
  begin
    ...
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 34 (sanity): anon/authenticated cannot call persist_ha_research_output at all: ' || 'anon role was able to call persist_ha_research_output', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 34 (sanity): anon/authenticated cannot call persist_ha_research_output at all: ' || 'anon role rejected at the privilege level', true, null);
  end;
  reset role;
  set role authenticated;
  begin
    perform public.persist_ha_research_output(gen_random_uuid(), gen_random_uuid(), 'x', gen_random_uuid(), null, null, null, null);
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 34 (sanity): anon/authenticated cannot call persist_ha_research_output at all: ' || 'authenticated role was able to call persist_ha_research_output directly', false, null);
  exception when insufficient_privilege then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 34 (sanity): anon/authenticated cannot call persist_ha_research_output at all: ' || 'authenticated role rejected at the privilege level', true, null);
  end;
  reset role;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 36: ' || 'expected status=completed in the response, got %', false, format('expected status=completed in the response, got %', v_result)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 36: ' || 'expected the row to be completed, got status=%, completed_at=%', false, format('expected the row to be completed, got status=%, completed_at=%', v_row.status, v_row.completed_at)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 36: ' || 'unexpected result_summary %', false, format('unexpected result_summary %', v_row.result_summary)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 37: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 38: ' || 'expected outcome=completed with the cached result_summary, got %', false, format('expected outcome=completed with the cached result_summary, got %', v_claim2)); end if;
  ...
  if v_claim2->>'outcome' = 'claimed-new' then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 39: ' || 'the manual rerun did not persist/finalize as expected: %', false, format('the manual rerun did not persist/finalize as expected: %', v_result)); end if;
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 39: ' || 'expected the manual rerun to claim successfully, got %', false, format('expected the manual rerun to claim successfully, got %', v_claim2)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 40 (sanity): wrong user / nonexistent upload rejected the same as every other RPC in this schema: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
...
begin
  ...
  exception when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 41: ' || 'the malformed accounts payload raised an exception (sqlstate %), aborting the whole call', true, format('the malformed accounts payload raised an exception (sqlstate %), aborting the whole call', sqlstate));
  end;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 42: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 43: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 44: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
...
begin
  ...
  exception when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 45: ' || 'a metadata-only accounts_maintenance edit after completion raised an unexpected exception: % (sqlstate %)', false, format('a metadata-only accounts_maintenance edit after completion raised an unexpected exception: % (sqlstate %)', sqlerrm, sqlstate));
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 45: ' || 'expected industry=New Industry after the maintenance edit, got %', false, format('expected industry=New Industry after the maintenance edit, got %', v_industry)); end if;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 46: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 47: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 48: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 48: ' || 'expected exactly 2 accounts to remain after the rejected calls, found %', false, format('expected exactly 2 accounts to remain after the rejected calls, found %', v_count)); end if;
...
begin
  ...
  exception when others then
    insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 49: ' || 'accounts_maintenance with no research history yet rejected a rename: % (sqlstate %)', false, format('accounts_maintenance with no research history yet rejected a rename: % (sqlstate %)', sqlerrm, sqlstate));
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 49: ' || 'expected the rename to succeed with no research history, got % matching rows', false, format('expected the rename to succeed with no research history, got % matching rows', v_count)); end if;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 50: ' || 'expected ok=false/reason=not-current-attempt, got %', false, format('expected ok=false/reason=not-current-attempt, got %', v_heartbeat)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 50: ' || 'lease_expires_at changed from % to % despite the rejection', false, format('lease_expires_at changed from % to % despite the rejection', v_lease_before, v_lease_after)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 50: ' || 'heartbeat_at changed from % to % despite the rejection', false, format('heartbeat_at changed from % to % despite the rejection', v_heartbeat_at_before, v_heartbeat_at_after)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 50: ' || 'expected outcome=reclaimed-after-expired-lease, got %', false, format('expected outcome=reclaimed-after-expired-lease, got %', v_claim->>'outcome')); end if;
  ...
  begin
    ...
    exception when others then
      ...
      else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 51: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
    end;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 52: ' || 'expected claimed-new, got %', false, format('expected claimed-new, got %', v_auto)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 52: ' || 'expected claimed-new for manual-active, got %', false, format('expected claimed-new for manual-active, got %', v_manual)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 52: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 53: ' || 'expected claimed-new for manual-2, got %', false, format('expected claimed-new for manual-2, got %', v_manual2)); end if;
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 53: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 54: ' || 'expected claimed-new for manual-done, got %', false, format('expected claimed-new for manual-done, got %', v_manual)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 54: ' || 'expected outcome=completed attached to manual-done, got %', false, format('expected outcome=completed attached to manual-done, got %', v_auto)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 55: ' || 'expected outcome=completed attached to manual-done, got %', false, format('expected outcome=completed attached to manual-done, got %', v_auto)); end if;
...
begin
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 56: ' || 'expected reclaimed-after-expired-lease, got %', false, format('expected reclaimed-after-expired-lease, got %', v_reclaimed)); end if;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 57: ' || 'expected reclaimed-after-failure, got %', false, format('expected reclaimed-after-failure, got %', v_retry)); end if;
...
begin
  ...
  exception when others then
    ...
    else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 58: ' || 'rejected, but with unexpected sqlstate % (message: %)', false, format('rejected, but with unexpected sqlstate % (message: %)', sqlstate, sqlerrm)); end if;
  end;
  ...
  else insert into _phase2a_auth_test_results (assertion, passed, detail) values ('Test 58: ' || '% row(s) were written despite the rejection', false, format('% row(s) were written despite the rejection', v_count)); end if;
...
-- ===========================================================================
-- Final safety-net cleanup + row-count restoration check. Every block above
-- already deletes its own fixtures inline; this is a belt-and-suspenders
-- sweep on top of that, not a replacement for it -- if any block above
-- aborted before reaching its own cleanup (a genuinely unexpected error, as
-- opposed to the deliberately-tested rejection paths, which are all caught
-- locally), this catches the orphaned rows anyway. Every fixture user in
-- this file uses an email starting with 'phase2a-', and ha_uploads/
-- ha_accounts/ha_signals/ha_research_runs all reference ha_users/ha_uploads
-- with ON DELETE CASCADE, so deleting by that email prefix is sufficient.
-- ===========================================================================
do $$
declare
  ...
  v_restored boolean;
begin
  delete from public.ha_users where email like 'phase2a-%@example.com';
  ...
  insert into _phase2a_auth_test_results (assertion, passed, detail) values (
    'Final: starting row counts restored exactly after full cleanup sweep',
    v_restored,
...
select * from _phase2a_auth_test_results order by id;

drop table if exists _phase2a_auth_test_results;
drop table if exists _phase2a_auth_baseline_counts;
