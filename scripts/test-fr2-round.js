...
// ---------------------------------------------------------------------------
const isHelpRelatedSrc = extractLines('isHelpRelatedGuidedTourTrigger', 4018, 4021, 'function isHelpRelatedGuidedTourTrigger(el){');
assert(
...
const launchGuidedTourSrc = extractLines('launchGuidedTour', 4023, 4056, 'function launchGuidedTour(){');
assert(
...
const closeGuidedTourSrc = extractLines('closeGuidedTour', 4058, 4102, 'function closeGuidedTour(){');
assert(
...
// ---------------------------------------------------------------------------
const safeParseSrc = extractLines('safeParseResearchResponse', 5729, 5764, 'async function safeParseResearchResponse(res, context){');
assert(
...
const researchAccountModalSrc = extractLines('researchAccountFromManageModal', 5766, 5904, 'async function researchAccountFromManageModal(accountName, listId){');
assert(
...
const researchListModalSrc = extractLines('researchListFromManageModal', 5919, 6087, 'async function researchListFromManageModal(listId){');
assert(
...
assert(
  /\}catch\(err\)\{\s*failures \+= 1;\s*\}/.test(researchListModalSrc),
...
// ---------------------------------------------------------------------------
const buildingPlaceholderSrc = extractLines('showBuildingDashboardPanelPlaceholder', 3539, 3550, 'function showBuildingDashboardPanelPlaceholder(){');
assert(
...
const fetchAggregateSrc = extractLines('fetchAndRenderAggregateDashboard', 3591, 3696, "async function fetchAndRenderAggregateDashboard(email, {silent = false} = {}){");
assert(
...
const processDataSrc = extractLines('processData', 9546, 9761, 'function processData(records){');
assert(
...
const retryStateSrc = extractLines('showDashboardRefreshRetryState', 3557, 3577, 'function showDashboardRefreshRetryState(){');
assert(
...
// ---------------------------------------------------------------------------
const handleListResearchClickSrc = extractLines('handleListResearchClick', 10169, 10264, 'async function handleListResearchClick(btn){');
assert(
