...
const dashboardHtml = loadDashboardSource();
const DASHBOARD_SRC = dashboardHtml;

// ---------------------------------------------------------------------------
// Item 1: Help stays closed after the guided tour ends (Finish/Skip/Escape),
// and focus never returns to a Help-menu trigger.
// ---------------------------------------------------------------------------
const isHelpRelatedSrc = extractFn(DASHBOARD_SRC, 'isHelpRelatedGuidedTourTrigger');
assert(
...
const launchGuidedTourSrc = extractFn(DASHBOARD_SRC, 'launchGuidedTour');
assert(
...
const closeGuidedTourSrc = extractFn(DASHBOARD_SRC, 'closeGuidedTour');
assert(
...
// ---------------------------------------------------------------------------
const safeParseSrc = extractFn(DASHBOARD_SRC, 'safeParseResearchResponse');
assert(
...
const researchAccountModalSrc = extractFn(DASHBOARD_SRC, 'researchAccountFromManageModal');
assert(
...
const researchListModalSrc = extractFn(DASHBOARD_SRC, 'researchListFromManageModal');
assert(
...
// ---------------------------------------------------------------------------
const buildingPlaceholderSrc = extractFn(DASHBOARD_SRC, 'showBuildingDashboardPanelPlaceholder');
assert(
...
const fetchAggregateSrc = extractFn(DASHBOARD_SRC, 'fetchAndRenderAggregateDashboard');
assert(
...
const retryStateSrc = extractFn(DASHBOARD_SRC, 'showDashboardRefreshRetryState');
assert(
