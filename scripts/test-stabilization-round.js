...
const loadSavedDashboardSrc = extractLines('fetchAndRenderAggregateDashboard', 3593, 3698, "async function fetchAndRenderAggregateDashboard(email, {silent = false} = {}){");
assert(
...
const addDataModalSrc = extractLines('add-customer-data-modal-and-route', 3340, 3463, 'let addCustomerDataModalTriggerEl = null;');
assert(
...
// ---------------------------------------------------------------------------
const uploadSuccessWiringSrc = extractLines('upload-success-wiring', 9858, 9883, 'function wireUploadSuccessStateControls(){');
assert(
...
const accountManagerOpenCloseSrc = extractLines('account-manager-open-close', 10080, 10103, 'let accountManagerTriggerEl = null;');
assert(
...
// ---------------------------------------------------------------------------
{
  const panelSrc = extractLines('showCustomerDashboardPanel', 3298, 3309, 'function showCustomerDashboardPanel({subtitleText, newCount, accountCount, signalCount, lastScanLabel}){');
...
// cross-list scoping.
// ---------------------------------------------------------------------------
const accountRowSrc = extractLines('accountRow', 9945, 9986, 'function accountRow(list, a){');
assert(
...
const researchFromModalSrc = extractLines('researchAccountFromManageModal', 5839, 5977, 'async function researchAccountFromManageModal(accountName, listId){');
assert(
...
const renderSwitcherSrc = extractLines('renderDashboardViewSwitcher', 3493, 3507, 'function renderDashboardViewSwitcher(){');
assert(
