...
const loadSavedDashboardSrc = extractLines('fetchAndRenderAggregateDashboard', 3610, 3715, "async function fetchAndRenderAggregateDashboard(email, {silent = false} = {}){");
assert(
...
const addDataModalSrc = extractLines('add-customer-data-modal-and-route', 3357, 3480, 'let addCustomerDataModalTriggerEl = null;');
assert(
...
// ---------------------------------------------------------------------------
const uploadSuccessWiringSrc = extractLines('upload-success-wiring', 10011, 10036, 'function wireUploadSuccessStateControls(){');
assert(
...
const accountManagerOpenCloseSrc = extractLines('account-manager-open-close', 10233, 10256, 'let accountManagerTriggerEl = null;');
assert(
...
// ---------------------------------------------------------------------------
{
  const panelSrc = extractLines('showCustomerDashboardPanel', 3315, 3326, 'function showCustomerDashboardPanel({subtitleText, newCount, accountCount, signalCount, lastScanLabel}){');
...
// cross-list scoping.
// ---------------------------------------------------------------------------
const accountRowSrc = extractLines('accountRow', 10098, 10139, 'function accountRow(list, a){');
assert(
...
const researchFromModalSrc = extractLines('researchAccountFromManageModal', 5925, 6063, 'async function researchAccountFromManageModal(accountName, listId){');
assert(
...
const renderSwitcherSrc = extractLines('renderDashboardViewSwitcher', 3510, 3524, 'function renderDashboardViewSwitcher(){');
assert(
