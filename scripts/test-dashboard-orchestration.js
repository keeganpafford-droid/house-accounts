...
const REAL_SOURCE = [
  extractFn('claimAutomaticResearchRun', 2226, 2236, {async: true}),
  extractFn('heartbeatCurrentResearchRun', 2256, 2273, {async: true}),
  extractFn('reportResearchRunOutcome', 2287, 2312, {async: true}),
  extractFn('normalizeSavedAccount', 2428, 2490),
  extractFn('accountCardFor', 3436, 3438),
  extractFn('accountSignalsPanel', 3439, 3442),
  extractFn('fetchUploadScopedSnapshot', 4296, 4318, {async: true}),
  extractFn('persistScopedResearchResult', 4326, 4371, {async: true}),
  extractFn('researchAccountFromManageModal', 4421, 4525, {async: true}),
  extractFn('researchAccountByName', 4545, 4685, {async: true}),
  extractFn('getAccountsForResearch', 4690, 4703),
  extractFn('batchPayloadForAccounts', 4705, 4749),
  extractFn('applyBusinessSignalAccountBoost', 4752, 4760),
  extractFn('researchAccountsBatch', 4762, 4851, {async: true}),
  extractFn('signalTopicKeyClient', 4853, 4861),
  extractFn('dedupeSignalsClient', 4863, 4876),
  extractFn('researchTopAccounts', 4878, 5008, {async: true}),
  extractFn('refreshOpportunityViews', 5109, 5129),
  extractFn('renderDetailedAccountViews', 6952, 7020),
  extractFn('serializeAccountForStorage', 7030, 7089),
  extractFn('performSaveCurrentUpload', 7099, 7199, {async: true}),
  extractFn('saveCurrentUpload', 7208, 7212),
  extractFn('toggleAccountMetadataEdit', 7220, 7226),
  extractFn('saveAccountMetadataEdit', 7249, 7285, {async: true}),
  extractRaw('delegatedClickListener', 7303, 7325, "document.addEventListener('click', (event) => {"),
  extractFn('importedContactsFromRecords', 7338, 7354),
