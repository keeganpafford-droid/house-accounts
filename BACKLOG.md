...
### Account Expansion / Whitespace Intelligence — Buying Center × Offering Matrix — COMPLETED / BANKED (2026-08-18, corrected 2026-08-19)
...
**Founder correction (2026-08-19), after independent Production verification of the initial ship (`99b9fa1`):** approved the overall UX direction but held back banking on two trust/architecture issues, corrected in the same slice before Active Expansion Play work begins:
1. **Persistence.** The buying-center mapping prompt's confirmation was client-local (localStorage) — organization/account intelligence, not a browser preference. Promoted to a durable, organization-scoped, cross-device Supabase table (see "Durable persistence" below), superseding the "Architecture decision, not yet implemented" note this entry originally carried.
2. **"Covered" truth standard.** The original rule (below) turned out to be too permissive on real data: a contact repeated across many/all order rows (a common real-world export shape) would satisfy same-row co-occurrence for every category that account ever purchased, even though the source data never actually proves that specific contact was linked to that specific purchase. Corrected to a per-contact category-discrimination standard — see "Corrected Covered standard" below. The row-level "known relationship" doctrine bullet is unchanged; only which cells that relationship is allowed to back changed.
...
- **Corrected Covered standard (per-contact category discrimination, 2026-08-19).** Same-row co-occurrence alone is insufficient: a contact can be a static/repeated per-account default appearing on every row regardless of category, which would otherwise falsely "cover" every category the account ever bought. A `(buying center, category)` cell reaches Covered only when a SPECIFIC transaction-evidenced contact's own evidenced rows span exactly one category — i.e. that contact is genuinely discriminating, never spread across multiple categories. A contact whose rows span multiple categories still contributes real row-level "known relationship" metadata but backs no specific cell (worked example: an account-wide HR contact appearing on both Apparel and Drinkware rows proves "account buys Apparel" and "account has an HR contact," never "HR bought Apparel" — HR × Apparel stays whitespace/unassigned until a rep confirms it or an integration supplies real linkage). Two different contacts within the same department are evaluated independently — one narrow (single-category) contact can still cover a cell even if another contact in the same department is a repeated/general one.
- **Known-relationship context is row-level metadata, not a cell state.** Knowing a contact in a buying center describes that row's label (e.g. "Jane Doe · known contact"), never paints every cell in that row.
...
- **Only real-evidence states render in Production.** Covered and Whitespace are the only two cell states ever assigned by real computation today. Not-Applicable and Active-Expansion-Play have reserved markup/CSS (explicit "N/A"/"EXPAND" text, no mystery icons) but are never assigned without a real confirmation mechanism or grounded expansion-play logic — no synthetic/demo cells in a real account. Internally, the data model still distinguishes Potential vs. Confirmed whitespace for future confidence/ranking use even though the UI intentionally collapses both to one quiet "Whitespace" treatment.
- **Unattributed purchases are never a fake buying-center row.** Real category evidence with no attributable department renders in a separate "Account-wide purchases not yet attributed" panel, outside the organizational grid.
...
- `computeAccountWhitespaceMatrix(account, confirmedCenters)` — pure function producing the real matrix using the corrected per-contact-discrimination Covered standard above, `whitespace` everywhere else, per-row known-contact metadata, and the unattributed-category list.
...
- **Explicitly not built yet:** grounded Active Expansion Play generation, any change to Relationship Expansion opportunity-generation logic, cell-level (as opposed to buying-center-level) confirm/correct. Full suite: 160/160.
...
**Priority: near-term — the Buying Center × Offering matrix is shipped and banked, see "Account Expansion / Whitespace Intelligence — Buying Center × Offering Matrix" under Recently completed above.** No Behavioral Learning dependency — this track reuses the existing contact/order/category foundation directly.
...
- **Cell-level confirm/correct.** Durable, org-shared buying-center-level confirmation persistence shipped 2026-08-19 (see the banked entry above — `ha_whitespace_confirmations` / `api/whitespace-map.js`). Remaining: extend beyond the buying-center-level prompt to real cell-level confirm/correct (Covered / Whitespace / Confirmed whitespace / Not applicable) once the row-level flow has real usage to learn from.
