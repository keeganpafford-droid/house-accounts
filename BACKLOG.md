...
- Activation remains fail-closed until explicitly enabled: both `NOTIFICATION_ENABLED_ORGANIZATION_IDS` (this feature) and `QUEUE_MANAGED_ORGANIZATION_IDS` (Queue-based monitoring, Phase 2D) are empty/unset in Production as of this banking — no Beta organization receives live notifications or Queue-based monitoring yet. Turning them on is a separate, explicit, founder-approved Production cutover, not a side effect of this merge.

**Important limitation — preserve, do not overstate:** House Accounts captures durable behavioral evidence today (`ha_signal_events`: feedback, selections, outreach, approach notes, and now outcome reports), but **Behavioral Learning does not yet exist** — none of this accumulated evidence currently changes future recommendation ranking, account prioritization, signal-type weighting, or rep/org behavioral profiles. Do not claim otherwise in product messaging, sales conversations, or future work until Behavioral Learning V1 (see NEXT below) is actually built. That item remains high-priority, near-term, sequenced immediately after monitoring + notification Production activation — it has not been started.
