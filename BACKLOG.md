...
### Account Expansion / Whitespace Intelligence — Buying Center × Offering Matrix — COMPLETED / BANKED (2026-08-18)
...
- **Same-row co-occurrence only.** A cell reaches Covered only when a SINGLE transaction-evidenced order row carries both a classifiable buying-center (contact department/title) and that same row's own category — never two independent account-level facts (a department contact somewhere + a category purchase somewhere, on different rows) stitched together.
...
- `computeAccountWhitespaceMatrix(account, confirmedCenters)` — pure function producing the real matrix: same-row-linked `covered` cells, `whitespace` everywhere else, per-row known-contact metadata, and the unattributed-category list.
- The "🗺️ Account Whitespace" matrix UI on the existing per-account expandable card (Manage Customer Accounts) — clean horizontal wrapped column headers (no rotated all-caps), no legend, CSS-grid tiles rather than a raw table.
- **Lightweight buying-center mapping prompt** for the current real-world common case (zero classifiable buying centers): "Help House Accounts map this customer" + clickable buying-center chips, matching the doctrine that HA builds the map wherever it has evidence and the rep supplies only small corrections — never per-cell manual data entry. Confirmation is **client-local only** (localStorage, mirroring `dismissedResearchNamespace()`'s established per-user namespacing pattern) — NOT the durable, cross-device, org-shared persistence layer described in the architecture decision below, which remains a future slice.
...
- **Explicitly not built yet:** durable/cross-device confirmation persistence, grounded Active Expansion Play generation, any change to Relationship Expansion opportunity-generation logic. Full suite: 159/159.
...
- **Durable confirmation persistence.** The shipped lightweight buying-center mapping prompt currently persists client-locally only (localStorage) — it does not survive a device change or reach the rest of the org. Promote it to the durable, cross-device, org-shared layer: key by the existing durable `(organization/user scope, normalizeCompanyName(account_name))` identity convention, not raw account name; use a small dedicated table, not `ha_signal_events` (architecture decision already made, see the banked entry above). Also extend beyond the buying-center-level prompt to real cell-level confirm/correct (Covered / Whitespace / Confirmed whitespace / Not applicable) once the row-level flow has real usage to learn from.
