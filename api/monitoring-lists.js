...
export default async function handler(req,res){try{const ctx=await context(req);if(!ctx)return json(res,401,{error:'Authentication required'});if(req.method==='GET'){
  ...
  // re-researched by the Queue monitoring architecture's recurring cadence
  // -- prospect accounts are one-shot research snapshots (see
  // api/lib/entitlement.js's usageFor()). monitoringStatus/monitoringCadence
  // must reflect that: they used to fold activeProspects into "Active"/
  // "next scan Monday" even when an org had zero active customer lists,
  // implying prospects are under the same continuous recurring watch
  // customer accounts get, which they are not. monitoringCadence (renamed
  // from nextWeeklyScan with the Full Beta Cutover) no longer names a fixed
  // day: monitoring-scheduler.js polls every 5 minutes for whichever
  // accounts are individually due, each on its own ~7-day-since-last-
  // success cycle, not one shared weekly clock for the whole organization.
  return json(res,200,{ok:true,scope:ctx.canViewTeam?'organization':'user',role:ctx.role,lists,summary:{activeCustomers,pausedCustomers,activeProspects,pausedProspects,monitoringCadence:activeCustomers>0?'Ongoing':null,monitoringStatus:activeCustomers>0?'Active':'No active lists'}})}if(req.method==='PATCH'){const{type,id,action,name}=req.body||{};if(type==='account'&&id&&['pause-account','resume-account','delete-account'].includes(action)){await patchCustomerAccount(ctx,id,action);return json(res,200,{ok:true})}if(!['customer','prospect'].includes(type)||!id||!['rename','pause','resume'].includes(action))return json(res,400,{error:'Invalid list update'});if(action==='rename'&&!clean(name))return json(res,400,{error:'List name is required'});await patchList(ctx,type,id,action,name);return json(res,200,{ok:true})}if(req.method==='DELETE'){const{type,id}=req.body||{};if(!['customer','prospect'].includes(type)||!id)return json(res,400,{error:'Invalid list delete'});await deleteList(ctx,type,id);return json(res,200,{ok:true})}return json(res,405,{error:'Method not allowed'})}catch(e){console.error('[Monitoring Lists]',e);
